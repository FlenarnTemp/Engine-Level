...
namespace RE
{
	namespace Cascadia
	{
		...
		void InitializeWeaponCondition(TESObjectREFR* weaponREFR)
		{
			...
			TESObjectWEAP* tempREFR = (TESObjectWEAP*)weaponREFR;

			if (tempREFR->weaponData.type == WEAPON_TYPE::kGrenade || tempREFR->weaponData.type == WEAPON_TYPE::kMine)
			{
				logger::debug("Weapon Condition: Returned due to grenade & mine weapon type.");
				return;
			}

			// Check for ExtraData.
			if (auto extraData = weaponREFR->extraList.get()) {
				// Check for existing kObjectHealth data.
				if (weaponREFR->extraList->HasType(kHealth))
				{
					logger::info(FMT_STRING("Weapon Condition: Non initialized data: {:s}"), std::to_string(weaponREFR->extraList.get()->GetHealthPercent()));
					return;
				}
				else
				{
					float setPercent = RNG(65.0f, 85.0f);
					logger::info(FMT_STRING("Weapon Condition: Newly initialized data: {:s}"), std::to_string(setPercent));

					extraData->SetHealthPercent(setPercent);
				}
			}
		}
	}
